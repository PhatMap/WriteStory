    ...
    safeGetItem("page4_state").then(savedState => {
      if (savedState) {
        try {
          ...
          console.error(texts.storyIllustration.parseErrorLog, e);
        }
      }
      setIsLoaded(true);
    });
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (msg.includes("Quota")) {
        setError(texts.storyIllustration.quotaError);
      } else if (msg.includes("an toàn") || msg.includes("safety") || msg.includes("PROHIBITED_CONTENT")) {
        setError(texts.storyIllustration.safetyError);
      } else {
        setError(`${texts.storyIllustration.genericErrorPrefix}: ${msg}. ${texts.storyIllustration.genericErrorSuffix}`);
      }
    } finally {
      setLoading(false);
    }
    ...
    <div className="max-w-4xl mx-auto p-4 sm:p-8">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-stone-900">{texts.storyIllustration.title}</h1>
        </div>
        <p className="text-stone-500 text-sm sm:text-base">{texts.storyIllustration.description}</p>
      </div>

      <div className="bg-white p-4 sm:p-6 rounded-2xl border border-stone-200 shadow-sm mb-8">
        <label className="block text-sm font-medium text-stone-700 mb-2">{texts.storyIllustration.promptLabel}</label>
        <textarea
          value={prompt}
          onChange={(e) => setPrompt(e.target.value)}
          placeholder={texts.storyIllustration.promptPlaceholder}
        ...
        <div className="flex justify-end">
          ...
          >
            ...
            {texts.storyIllustration.generateButton}
          </button>
        </div>
      </div>
      ...
      {loading && (
        <div className="flex flex-col items-center justify-center py-20 bg-stone-100 rounded-2xl border border-stone-200 border-dashed">
          ...
          <p className="text-stone-500 font-medium">{texts.storyIllustration.loadingText}</p>
        </div>
      )}
      ...
      {imageUrl && !loading && (
        <div className="bg-white p-4 rounded-2xl border border-stone-200 shadow-sm">
          <div className="relative group rounded-xl overflow-hidden">
            <img src={imageUrl} alt={texts.storyIllustration.generatedAlt} className="w-full h-auto object-cover" referrerPolicy="no-referrer" />
            <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
              ...
              >
                <Download size={18} />
                {texts.storyIllustration.downloadButton}
              </a>
            </div>
          </div>
        </div>
      ...
      <div className="mt-8 flex justify-start">
        <Link to="/editor" className="px-6 py-3 bg-white border border-stone-300 text-stone-700 rounded-xl font-medium hover:bg-stone-50 flex items-center gap-2 transition-colors">
          <ArrowLeft size={18} /> {texts.storyIllustration.backToEditor}
        </Link>
      </div>
    </div>
  );
